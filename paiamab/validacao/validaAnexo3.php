<?php 	
	...
	if(is_bool($ficha)){
		$_SESSION['anexo3'] = $prontuario->getFormulario();
		$mensagem_tipo = 1;
		$mensagem_texto = "Anexo 3 Salvo Com Sucesso";
		
		$log = new Log();
			if($_POST['submit'] == "Salvar")
				$log->ProntuarioCriacao($_SESSION['id_usuario'], $usuario->get_nome(), $pos_n_controle, "o Anexo 3", $_SESSION['id_ficha']);
			if($_POST['submit'] == "Atualizar")
				$log->ProntuarioEdicao($_SESSION['id_usuario'], $usuario->get_nome(), $pos_n_controle, "o Anexo 3", $_SESSION['id_ficha']);
	}
	else{
		$mensagem_tipo = 0;
		$mensagem_texto = "Erro ao Salvar o Anexo 3.<br>Tente Novamente mais tarde.<br>".$ficha;
	}
